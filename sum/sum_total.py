...
#  sum_total.py
#  |--sum_total      - 画面作成
#  |--sum_total_exe  - 実行する、タスクオブジェクトを渡す
#  |--sum_total_task - タスク処理、sheetを作成、メール送信
#  sum_total_mod.py - sheetを作成
#  sum_total_sql.py - modを実行するときのsql
# -------------------------------------------------------------------
import io
import sys
import json
...
from _mod import mod_base, mod_que, mod_datetime, mod_gmail_api
...
from sum import sum_total_mod, sum_common_mod
...
def sum_total_exe():
    ...
    if level_error == "error":
        ...
        if task_sta == "end":
            ...
            que_url = fs_dic["que_site"] + "/sum/total_task"
            que_body = {
...
def sum_total_task():
    ...
    keiyaku_grp_name = js_obj["keiyaku_grp_name"]
    ...
    # section_list
    section_list = sum_common_mod.moz_section_list()
    for section_cd in section_list:
        ...
        res_fee_money_sum_grand_tou = 0
        res_fee_money_sum_grand_rui = 0
        ...
        # create sheet, setting, header, title, cat
        section_name = mod_section.mz_section_name4sum_total(section_cd)
        sheet = book.add_worksheet(section_name)
        sheet = sum_common_mod.mz_setting(sheet)
        sheet, row = sum_common_mod.mz_header(
            sheet,
            row,
            cf_dic,
            send_file_title_header + "-" + keijyo_date_str + "-" + section_name + "-" + keiyaku_grp_name,
        )
        ...
        for cat_cd in cat_list:
            # data
            sheet, row, data_dic = sum_total_mod.mz_data(
                sheet,
                row,
                cf_dic,
                keijyo_nendo,
            ...
            res_fee_money_sum_grand_tou += data_dic["res_fee_money_sum_tou"]
            res_fee_money_sum_grand_rui += data_dic["res_fee_money_sum_rui"]
        ...
        grand_dic = {
            ...
            "res_fee_money_sum_grand_tou": res_fee_money_sum_grand_tou,
            "res_fee_money_sum_grand_rui": res_fee_money_sum_grand_rui,
    ...
    sheet = sum_common_mod.mz_data_total_list(sheet, cf_dic, keijyo_date_int, keiyaku_grp_cd)
    ...
    # subject/body
    kensu_for_email = sum_common_mod.mz_data_total_kensu_for_email(keijyo_date_int, keiyaku_grp_cd)
    ...
    if service_gmail is not None:
        ...
        try:
            ...
            print(f"sum_total メール送信成功: {result}")
        except Exception as e:
            print(f"sum_total メール送信エラー: {e}")
            import traceback

            print(f"sum_total 詳細エラー: {traceback.format_exc()}")
