...
#  sum_list.py
#  |--sum_list      - 画面作成
#  |--sum_list_exe  - 実行する、タスクオブジェクトを渡す
#  |--sum_list_task - タスク処理、sheetを作成、メール送信
#  sum_list_mod.py - sheetを作成
#  sum_list_sql.py - modを実行するときのsql
# -------------------------------------------------------------------
import io
import sys
import json
...
def sum_list_exe():
    ...
    if level_error == "error":
        ...
        if task_sta == "end":
            ...
            que_url = fs_dic["que_site"] + "/sum/list_task"
            que_body = {
...
def sum_list_task():
    ...
    sheet = sum_common_mod.mz_data_list_list(sheet, cf_dic, keijyo_date_int)
    ...
    # subject/body
    kensu_for_email = sum_common_mod.mz_data_list_kensu_for_email(keijyo_date_int)
    ...
    if service_gmail is not None:
        ...
        try:
            ...
            print(f"sum_list メール送信成功: {result}")
        except Exception as e:
            print(f"sum_list メール送信エラー: {e}")
            import traceback

            print(f"sum_list 詳細エラー: {traceback.format_exc()}")
