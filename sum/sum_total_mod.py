...
def mz_data(
    ...
    res_fee_money_sum_tou = 0
    res_fee_money_sum_rui = 0
    ...
    # ----------------------------------------------------------------------------------------------------------------------------------
    # 当月 メインデータ
    sql_data = sum_total_sql_data.mz_data_tou(keijyo_date_int, keiyaku_grp_cd, cat_cd, section_cd)
    ...
    for dt in sql_data:
        ...
        sheet.write(row, 5, dt["res_fee_money"], cf_r14_comma_bor)
        ...
        if dt["cnt"] is not None:
            ...
            res_fee_money_sum_tou += int(dt["res_fee_money"])
    ...
    sheet.write(row_total, 5, res_fee_money_sum_tou, cf_r14_comma_bor)
    ...
    sql_data = sum_total_sql_data.mz_data_rui(keijyo_nendo, keiyaku_grp_cd, cat_cd, section_cd)

    for dt in sql_data:
        row += 1
        sheet.write(row, 10, dt["res_hoken_ryo_year"], cf_r14_comma_bor)
        sheet.write(row, 11, dt["res_fee_money"], cf_r14_comma_bor)
        ...
        if dt["cnt"] is not None:
            ...
            res_fee_money_sum_rui += int(dt["res_fee_money"])
    ...
    sheet.write(row_total, 11, res_fee_money_sum_rui, cf_r14_comma_bor)
    ...
    # 当月 契約件数
    row = sum_common_mod.moz_tori_back(cat_cd, row)  # 取り扱い保険会社件数分だけ、上に戻す
    sql_data = sum_total_sql_kensu.mz_data_tou_kensu(keijyo_date_int, keiyaku_grp_cd, cat_cd, section_cd)
    ...
    # 累計 契約件数
    row = sum_common_mod.moz_tori_back(cat_cd, row)  # 取り扱い保険会社件数分だけ、上に戻す
    sql_data = sum_total_sql_kensu.mz_data_rui_kensu(keijyo_nendo, keiyaku_grp_cd, cat_cd, section_cd)
    ...
    # 当月 契約顧客件数
    row = sum_common_mod.moz_tori_back(cat_cd, row)  # 取り扱い保険会社件数分だけ、上に戻す
    sql_data = sum_total_sql_keiko.mz_keiko_tou(keijyo_date_int, keiyaku_grp_cd, cat_cd, section_cd)
    ...
    # 累計 契約顧客件数
    row = sum_common_mod.moz_tori_back(cat_cd, row)  # 取り扱い保険会社件数分だけ、上に戻す
    sql_data = sum_total_sql_keiko.mz_keiko_rui(keijyo_nendo, keiyaku_grp_cd, cat_cd, section_cd)
    ...
    data_dic = {
        ...
        "res_fee_money_sum_tou": res_fee_money_sum_tou,
        "res_fee_money_sum_rui": res_fee_money_sum_rui,
