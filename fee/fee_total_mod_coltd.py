...
from fee import (
    ...
    fee_total_sql_coltd_gol,
    fee_total_sql_coltd_rui,
    fee_total_sql_coltd_tou,
)
...
def mz_data_sum(sheet, cf_dic, row, nyu_date, nyu_nendo):
    ...
    for dt in sec_data:
        ...
        sql_data = fee_total_sql_coltd_gol.mz_sql_fee_tou_sum(nyu_date, section_cd)
        for dt in sql_data:
    ...
    for dt in sec_data:
        ...
        sql_data = fee_total_sql_coltd_gol.mz_sql_fee_rui_sum(nyu_date, nyu_nendo, section_cd)
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
            res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    for dt in sec_data:
        ...
        for cdt in cat_data:
            sql_data = fee_total_sql_coltd_tou.mz_sql_fee_tou_list(nyu_date, cdt["cat_cd"], section_cd)
            for dt in sql_data:
                sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
                row += 1
            row += 1
    ...
    for dt in sec_data:
        ...
        for cdt in cat_data:
            sql_data = fee_total_sql_coltd_rui.mz_sql_fee_rui_list(nyu_date, nyu_nendo, cdt["cat_cd"], section_cd)
            for dt in sql_data:
                sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
                row += 1
            row += 1
