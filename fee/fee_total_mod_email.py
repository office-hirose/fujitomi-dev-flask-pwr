from fee import (
    ...
    fee_total_sql_email_gol,
    fee_total_sql_email_rui,
    fee_total_sql_email_tou,
)
...
def mz_data_sum(sheet, cf_dic, row, nyu_date, nyu_nendo):
    ...
    # sec_data = fee_total_sql_common.mz_sql_section_kyutou_nasi()
    ...
    for cdt in cat_data:
        ...
        row = start_row
        col += 1
        sql_data = fee_total_sql_email_gol.mz_tou_tan(nyu_date, cdt["cat_cd"])
        for dt in sql_data:
        ...
        row = start_row
        col += 1
        sql_data = fee_total_sql_email_gol.mz_tou_gyo(nyu_date, cdt["cat_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
            res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    row = start_row
    col += 1
    sql_data = fee_total_sql_email_gol.mz_tou_tan(nyu_date, "all")
    for dt in sql_data:
        sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
        row += 1
        res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    row = start_row
    col += 1
    sql_data = fee_total_sql_email_gol.mz_tou_gyo(nyu_date, "all")
    for dt in sql_data:
        sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
        row += 1
        res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    row = start_row
    col += 1
    sql_data = fee_total_sql_email_gol.mz_tou_total(nyu_date)
    for dt in sql_data:
        sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
        row += 1
        res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    for cdt in cat_data:
        ...
        row = start_row
        col += 1
        sql_data = fee_total_sql_email_gol.mz_rui_tan(nyu_date, nyu_nendo, cdt["cat_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
            res = mz_gtotal_calc(dt["pay_fee_yen"])
        ...
        row = start_row
        col += 1
        sql_data = fee_total_sql_email_gol.mz_rui_gyo(nyu_date, nyu_nendo, cdt["cat_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
            res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    row = start_row
    col += 1
    sql_data = fee_total_sql_email_gol.mz_rui_tan(nyu_date, nyu_nendo, "all")
    for dt in sql_data:
        sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
        row += 1
        res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    row = start_row
    col += 1
    sql_data = fee_total_sql_email_gol.mz_rui_gyo(nyu_date, nyu_nendo, "all")
    for dt in sql_data:
        sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
        row += 1
        res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    row = start_row
    col += 1
    sql_data = fee_total_sql_email_gol.mz_rui_total(nyu_date, nyu_nendo)
    for dt in sql_data:
        sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
        row += 1
        res = mz_gtotal_calc(dt["pay_fee_yen"])
    ...
    for sdt in sec_data:
        left_number = 1
        sql_data = fee_total_sql_common.mz_staff_data_email(sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, 0, left_number, cf_c14_bor)
    ...
    for cdt in cat_data:
        ...
        col += 1
        for sdt in sec_data:
            sql_data = fee_total_sql_email_tou.mz_tou_tan(nyu_date, cdt["cat_cd"], sdt["section_cd"])
            for dt in sql_data:
                sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
                row += 1
            row += 1
        ...
        col += 1
        for sdt in sec_data:
            sql_data = fee_total_sql_email_tou.mz_tou_gyo(nyu_date, cdt["cat_cd"], sdt["section_cd"])
            for dt in sql_data:
                sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
                row += 1
            row += 1
    ...
    col += 1
    for sdt in sec_data:
        sql_data = fee_total_sql_email_tou.mz_tou_tan(nyu_date, "all", sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
        row += 1
    ...
    col += 1
    for sdt in sec_data:
        sql_data = fee_total_sql_email_tou.mz_tou_gyo(nyu_date, "all", sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
        row += 1
    ...
    col += 1
    for sdt in sec_data:
        sql_data = fee_total_sql_email_tou.mz_tou_total(nyu_date, sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
        row += 1
    ...
    for cdt in cat_data:
        ...
        col += 1
        for sdt in sec_data:
            sql_data = fee_total_sql_email_rui.mz_rui_tan(nyu_date, nyu_nendo, cdt["cat_cd"], sdt["section_cd"])
            for dt in sql_data:
                sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
                row += 1
            row += 1
        ...
        col += 1
        for sdt in sec_data:
            sql_data = fee_total_sql_email_rui.mz_rui_gyo(nyu_date, nyu_nendo, cdt["cat_cd"], sdt["section_cd"])
            for dt in sql_data:
                sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
                row += 1
            row += 1
    ...
    col += 1
    for sdt in sec_data:
        sql_data = fee_total_sql_email_rui.mz_rui_tan(nyu_date, nyu_nendo, "all", sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
        row += 1
    ...
    col += 1
    for sdt in sec_data:
        sql_data = fee_total_sql_email_rui.mz_rui_gyo(nyu_date, nyu_nendo, "all", sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
        row += 1
    ...
    col += 1
    for sdt in sec_data:
        sql_data = fee_total_sql_email_rui.mz_rui_total(nyu_date, nyu_nendo, sdt["section_cd"])
        for dt in sql_data:
            sheet.write(row, col, dt["pay_fee_yen"], cf_r14_comma)
            row += 1
        row += 1
