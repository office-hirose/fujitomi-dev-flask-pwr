...
from fee import fee_cre_month_keiri_sql


# delete
def mz_del(nyu_date_int):
    fee_cre_month_keiri_sql.mz_sql_fee_order_store_del(nyu_date_int)
    return
...
def mz_cre(nyu_date_int):
    exe_cnt = 0
    sql_data = fee_cre_month_keiri_sql.mz_sql_fee_order_store(nyu_date_int)
    for dt in sql_data:
        ...
        for stf in stf_list:
            ...
            if insert_exe == "yes":
                ...
                with sql_con:
                    sql = """
                    INSERT INTO sql_fee_order_store_keiri (
                        nyu_nendo,
                        nyu_date,
                        kind_cd,
                        cat_cd,
...
def mz_update_kaime(nyu_date_int):
    ...
    with sql_con:
        sql = (
            "UPDATE sql_fee_order_store_keiri AS fos1,"
            + "(SELECT * FROM sql_fee_order_store_keiri WHERE kind_cd = 1 AND nyu_date = %s) AS fos2"
