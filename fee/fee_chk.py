...
def fee_chk_exe():
    ...
    if level_error == "error":
        ...
        if task_sta == "end":
            ...
            que_url = fs_dic["que_site"] + "/fee/chk_task"
...
def fee_chk_task():
    ...
    sheet = book.add_worksheet("証券番号リンクエラーリスト")

    # cell format dic
    cf_dic = mod_xlsxwriter.mz_cf(book)

    # チェックデータ作成
    sheet = fee_chk_mod.mz_title(sheet, cf_dic)
    sheet, sql_data_cnt = fee_chk_mod.mz_data(sheet, cf_dic, nyu_date_int)
    ...
    if service_gmail is not None:
        ...
        try:
            ...
            print(f"fee_chk メール送信成功: {result}")
        except Exception as e:
            print(f"fee_chk メール送信エラー: {e}")
            import traceback

            print(f"fee_chk 詳細エラー: {traceback.format_exc()}")
