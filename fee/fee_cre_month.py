...
#  fee_cre_month_mod.py  - 作成実行
#  fee_cre_month_sql.py  - 実行sql
# -------------------------------------------------------------------
import sys
import json
import base64
...
from fee import fee_cre_month_mod
...
def fee_cre_month_exe():
    ...
    if level_error == "error":
        ...
        if task_sta == "end":
            ...
            que_url = fs_dic["que_site"] + "/fee/cre_month_task"
...
def fee_cre_month_task():
    ...
    # delete
    fee_cre_month_mod.mz_del(nyu_date_int)

    # create, insert
    exe_cnt = fee_cre_month_mod.mz_cre(nyu_date_int)

    # 回目を修正する
    fee_cre_month_mod.mz_update_kaime(nyu_date_int)
    ...
    if service_gmail is not None:
        ...
        try:
            ...
            print(f"fee_cre_month メール送信成功: {result}")
        except Exception as e:
            print(f"fee_cre_month メール送信エラー: {e}")
            import traceback

            print(f"fee_cre_month 詳細エラー: {traceback.format_exc()}")
