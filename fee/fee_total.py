...
#  fee_total.py
#  |--fee_total      - 画面作成
#  |--fee_total_exe  - 実行する、タスクオブジェクトを渡す
#  |--fee_total_task - タスク処理、sheetを作成、メール送信
#  fee_total_mod.py  - sheetを作成
#  fee_total_sql.py  - modを実行するときのsql
# -------------------------------------------------------------------
import io
import sys
import json
...
def fee_total_exe():
    ...
    if level_error == "error":
        ...
        if task_sta == "end":
            ...
            que_url = fs_dic["que_site"] + "/fee/total_task"
            que_body = {
...
def fee_total_task():
    ...
    # 1 保険会社別 税抜 代理店手数料のみ
    title_name_add_notax_daifee_only = "(税抜/代理店手数料のみ)"
    row = 0
    sheet = book.add_worksheet("保険会社別_税抜_代理店手数料のみ")
    sheet = fee_total_mod_keiri.mz_setting_sum(sheet)
    sheet, row = fee_total_mod_keiri.mz_title_sum(
        sheet, cf_dic, row, title_name + title_name_add_notax_daifee_only, nyu_nendo_str
    )  # タイトル名に追加
    sheet, row = fee_total_mod_keiri.mz_data_sum(sheet, cf_dic, row, nyu_date_int, nyu_nendo_int)
    row = row - 2
    sheet, row = sum_common_mod.mz_datetime_user(sheet, row, 16, cf_dic, user_email)

    # 2 保険会社別 税抜 振替費用含む
    title_name_add_notax_furikae_fukumu = "(税抜/振替費用含む)"
    row = 0
    sheet = book.add_worksheet("保険会社別_税抜_振替費用含む")
    sheet = fee_total_mod_coltd.mz_setting_sum(sheet)
    sheet, row = fee_total_mod_coltd.mz_title_sum(
        sheet, cf_dic, row, title_name + title_name_add_notax_furikae_fukumu, nyu_nendo_str
    )  # タイトル名に追加
    sheet, row = fee_total_mod_coltd.mz_data_sum(sheet, cf_dic, row, nyu_date_int, nyu_nendo_int)
    row = row - 2
    sheet, row = sum_common_mod.mz_datetime_user(sheet, row, 16, cf_dic, user_email)

    # 3 担当別（Email別なので担当は、まとめて表示される）
    row = 0
    sheet = book.add_worksheet("担当別")
    sheet = fee_total_mod_email.mz_setting_sum(sheet)
    sheet, row = fee_total_mod_email.mz_title_sum(sheet, cf_dic, row, title_name, nyu_nendo_str)
    sheet, row = fee_total_mod_email.mz_data_sum(sheet, cf_dic, row, nyu_date_int, nyu_nendo_int)
    row = row - 2
    sheet, row = sum_common_mod.mz_datetime_user(sheet, row, 20, cf_dic, user_email)

    # 4 担当別（staff_cdで分けているので、セクションに複数の担当が表示される）
    row = 0
    sheet = book.add_worksheet("担当別_複数拠点")
    sheet = fee_total_mod_stf.mz_setting_sum(sheet)
    sheet, row = fee_total_mod_stf.mz_title_sum(sheet, cf_dic, row, title_name, nyu_nendo_str)
    sheet, row = fee_total_mod_stf.mz_data_sum(sheet, cf_dic, row, nyu_date_int, nyu_nendo_int)
    row = row - 2
    sheet, row = sum_common_mod.mz_datetime_user(sheet, row, 20, cf_dic, user_email)
    ...
    sheet = fee_total_mod_list.mz_title_list(sheet, cf_dic)
    sheet, sql_data_cnt = fee_total_mod_list.mz_data_list_stf(sheet, row, cf_dic, nyu_date_int)

    # 6 生保マイナスリスト
    row = 0
    sheet = book.add_worksheet("生保マイナスリスト")
    sheet = fee_total_mod_list_minus.mz_title_list(sheet, cf_dic)
    sheet = fee_total_mod_list_minus.mz_data_list_stf(sheet, row, cf_dic, nyu_date_int)
    ...
    if service_gmail is not None:
        ...
        try:
            ...
            print(f"fee_total メール送信成功: {result}")
        except Exception as e:
            print(f"fee_total メール送信エラー: {e}")
            import traceback

            print(f"fee_total 詳細エラー: {traceback.format_exc()}")
