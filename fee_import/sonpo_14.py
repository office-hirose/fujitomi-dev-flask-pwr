...
def set_sheet(sheet_dic):
    ...
    org_list_tori_num = original_sheet.col_values(tori_num)[y:]
    ...
    new_list_tori_num = new_sheet.range(f"N2:N{row_len}")  # 取扱手数料
    ...
    for i in range(row_len - 1):
        ...
        # 取扱手数料 N
        try:
            new_list_tori_num[i].value = int(org_list_tori_num[i].replace(",", ""))
        except ValueError:
            print(f"org_list_tori_num convert error '{org_list_tori_num[i]}' to a number")

        # 手数料種類 E
        if int(new_list_tori_num[i].value) == 0:
    ...
    new_sheet.update_cells(new_list_tori_num)
