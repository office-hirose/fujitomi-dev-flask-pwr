# ------------------------------------------------------------------------
#  keijyo_list.py
#  |--keijyo_list_all      - 画面作成
#  |--keijyo_list_all_exe  - 実行する、タスクオブジェクトを渡す
#  |--keijyo_list_all_task - タスク処理、sheetを作成、メール送信
#  keijyo_list_all_mod.py - sheetを作成
#  keijyo_list_all_sql.py - modを実行するときのsql
# ------------------------------------------------------------------------
import io
import sys
import json
...
from keijyo import keijyo_list_all_mod
...
def keijyo_list_all_exe():
    ...
    if level_error == "error":
        ...
        que_url = fs_dic["que_site"] + "/keijyo/list_all_task"
        que_body = {
...
def keijyo_list_all_task():
    ...
    sheet = book.add_worksheet(send_file_title_header)
    ...
    # sheet title
    row = 0
    sheet, row = keijyo_list_all_mod.mz_title(sheet, row, cf_dic)

    # sheet data
    sheet, row, sql_data_cnt = keijyo_list_all_mod.mz_data(keijyo_date_int, sheet, row, cf_dic)
    ...
    body_data += "計上月　：" + keijyo_date_str + "以降" "\n"
    ...
    if service_gmail is not None:
        ...
        try:
            ...
            print(f"keijyo_list_all メール送信成功: {result}")
        except Exception as e:
            print(f"keijyo_list_all メール送信エラー: {e}")
            import traceback

            print(f"keijyo_list_all 詳細エラー: {traceback.format_exc()}")
